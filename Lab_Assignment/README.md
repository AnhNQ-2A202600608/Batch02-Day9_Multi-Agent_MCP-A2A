...
### Kiến Trúc Multi-Agent Supervisor-Worker

Phiên bản chatbot hiện tại hỗ trợ mô hình `supervisor -> 2-3 worker -> critic -> synthesizer`:

- `Supervisor`: phân loại query thành `legal`, `news`, `hybrid`, hoặc `cross_domain`, rồi giao việc cho worker phù hợp.
- `Legal Worker`: truy xuất chunk pháp luật từ semantic, lexical và hybrid retrieval.
- `News Worker`: truy xuất chunk báo chí liên quan nghệ sĩ hoặc vụ việc.
- `Hybrid Worker`: chạy retrieval pipeline tổng quát để bổ sung evidence xuyên miền.
- `Critic`: khử trùng lặp, gắn trọng số theo miền và chọn top-k chunks cuối.
- `Synthesizer`: sinh câu trả lời cuối cùng với citation từ context đã được duyệt.

Quy tắc giao việc:

- Route `legal`: chạy `legal_worker` + `hybrid_worker`
- Route `news`: chạy `news_worker` + `hybrid_worker`
- Route `hybrid` hoặc `cross_domain`: chạy cả 3 worker song song

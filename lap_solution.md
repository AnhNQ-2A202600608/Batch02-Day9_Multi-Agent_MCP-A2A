...
## 8. CÁC CẢI TIẾN TRÊN GIAO DIỆN WEB VISUALIZER DASHBOARD (`index.html`)
...
* **Cơ chế Fast-Forward (Tăng tốc hoạt ảnh):**
  * Nếu API phản hồi nhanh hoặc kết quả được cache trả về trước khi hoạt ảnh kết thúc, hàm `awaitSleep` sẽ tự động chuyển sang trạng thái tăng tốc, chỉ ngủ tối đa `500ms` cho mỗi bước tiếp theo.

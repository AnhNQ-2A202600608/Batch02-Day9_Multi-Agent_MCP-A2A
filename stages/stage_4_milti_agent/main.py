    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data privacy, GDPR, CCPA, data breaches, user data'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a specialist data privacy attorney with expertise in GDPR, CCPA, "
        "data protection regulations, data breach notification laws, and user consent frameworks. "
        "Use the search_compliance_law tool (which has GDPR/CCPA data) to ground your analysis. Keep your response under 200 words."
    )
...
async def aggregate(state: LegalState) -> dict:
    ...
    if state.get("privacy_result"):
        sections.append(f"## Data Privacy & GDPR Analysis\n{state['privacy_result']}")
...
QUESTION = (
    "A technology company suffered a severe data breach leaking user records. "
    "They also broke an NDA contract and avoided taxes. What are the legal, "
